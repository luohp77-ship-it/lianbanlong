...
def analyze_concepts_by_reason(up_stocks, callback=None):
    ...
    """
    if callback:
        callback('  概念分析: 基于 reason_type 分组...')
    ...
    if callback:
        if ranked:
            ...
            callback('  概念分析: 完成, TOP3: %s' % top_names)
        else:
...
def analyze_concepts(limit_up_codes, callback=None, up_stocks=None):
    """概念分析（push2优先，reason_type降级）。

    优先使用push2 API获取概念板块数据，不可用时降级到
    同花顺reason_type分组分析。

    Args:
        limit_up_codes: 今日涨停股代码集合。
        callback: 日志回调函数。
        up_stocks: 涨停股列表（reason_type降级时使用），
                   每个元素需含 code/name/reason_type 字段。
    ...
    # push2不可用时，降级到reason_type分组
    if not ranked and up_stocks:
        if callback:
            callback('  概念分析: push2不可用，降级使用reason_type分组...', level='WARN')
        result_by_reason = analyze_concepts_by_reason(up_stocks, callback=None)
        ranked = result_by_reason.get('ranked', [])
        code_concepts = result_by_reason.get('code_concepts', {})
