#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""复盘引擎 V2.1：数据获取 → 连板计算(TDX本地) → 概念分析(reason_type) → 板块写入
...
def run_engine(config=None, callback=None):
    ...
    # ── 5. 概念分析（push2优先，reason_type降级 + 综合评分重排）──
    log('  概念分析(push2优先+综合评分)...')
    ...
    # 涨停类板块（来自up_stocks，包含ST补充）
    _write_blk_verified(os.path.join(blocknew_dir, 'ZFB.blk'),
                        [str(s.get('code', '')).zfill(6) for s in up_stocks],
                        'ZFB', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, 'SB.blk'), sb, 'SB', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, '2LB.blk'), lb2, '2LB', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, '3LB.blk'), lb3, '3LB', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, '4LB.blk'), lb4, '4LB', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, '5BYS.blk'), lb5, '5BYS', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, 'SYLB.blk'), sylb, 'SYLB', code_name_map)
    ...
    _write_blk_verified(os.path.join(blocknew_dir, 'RDGN.blk'),
                        top1_codes, 'RDGN', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, 'HYRD.blk'),
                        top2_codes, 'HYRD', code_name_map)
    _write_blk_verified(os.path.join(blocknew_dir, 'RDGN3.blk'),
                        top3_codes, 'RDGN3', code_name_map)

    # 昨涨停今（ZFB2）
    # 优先用boards缓存，无缓存则用同花顺API获取昨日涨停
    ...
    if n_extern:
        ...
        log('  自定义列: %d只涨停股, %d只有原因, %d只有概念' % (
            n_extern, n_with_reason, n_with_concept))
    ...
    log('--- 完成: %d涨停(%d首板/%d连板) 最高%d板 %d跌停 %d曾涨停 %d曾跌停 %d龙虎榜 溢价%.1f%% 炸板%.0f%% ---' % (
...
def install_blocks(config=None, callback=None, concept_names=None):
    ...
    # 写入 gridtab.dat
    new_data = bytearray()
    sector_rids = set(range(0x9026, 0x9037 + 1))
    sector_rids.discard(0x9035)
...
def _install_sp(tdx_dir, blocknew_dir):
    ...
    funcs_dir = os.path.join(tdx_dir, 'funcs')
    os.makedirs(funcs_dir, exist_ok=True)
    dst = os.path.join(funcs_dir, SP_FILENAME)
    shutil.copy2(src, dst)
    log('  看盘界面已安装: %s' % dst)
...
def _update_sp(tdx_dir, concept_names):
    ...
    funcs_dir = os.path.join(tdx_dir, 'funcs')
    sp_path = os.path.join(funcs_dir, SP_FILENAME)
