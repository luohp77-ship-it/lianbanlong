...
def set_column_green(tdx_dir):
    """将通达信自定义数据列（数据号1-3）的字体颜色设为荧光绿，缺失列自动创建。
    ...
    for did, name in NEW_DEFS.items():
        if did not in existing_ids:
            ...
            # byte 0x1c (28): color = green
            _struct.pack_into('<I', rec, 0x1c, GREEN)
            data.extend(rec)
            log('  [自动创建] 自定义列 数据号%d: %s' % (did, name))

    # Step 3: 为所有数据号1-3设置绿色
    n = 0
    for i in range(len(data) // RECORD_SIZE):
        off = i * RECORD_SIZE
        d_id = _struct.unpack_from('<I', data, off + 4)[0]
        if d_id in ALL_IDS:
            _struct.pack_into('<I', data, off + 0x1c, GREEN)
            n += 1
    ...
    if n:
        ...
        log('  自定义列颜色已设为荧光绿: %d条记录 (数据号1-3)' % n)
        return True
