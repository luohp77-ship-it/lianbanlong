#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""复盘助手 V2.1.2 - 通达信自动复盘桌面应用

V2.1.2变更（Bug修复）：
- 修复概念分析TypeError崩溃，概念分析恢复可用
...
class FupanApp:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title('复盘助手 V2.1.2 - 通达信自动复盘')
    ...
    def _refresh(self):
        ...
        try:
            ...
            if fp.exists():
                ...
                self.mv['lhb'].config(text=str(data.get('lhb', '-')))
                # V5新增卡片
                promo = data.get('promoRate', 0)
                self.mv['promo'].config(text='%.0f%%' % promo if promo else '-')
                premium = data.get('premiumRate', 0)
                self.mv['premium'].config(text='%.1f%%' % premium if premium else '-')
                zhaban = data.get('zhabanRate', 0)
                self.mv['zhaban'].config(text='%.0f%%' % zhaban if zhaban else '-')
    ...
    def _open_settings(self):
        ...
        tk.Entry(f, textvariable=key_var, width=40,
        ...
        # V5新增：TG Bot配置
        tg_var = tk.BooleanVar(value=cfg.get('enableTgBot', False))
        tk.Checkbutton(f, text='启用 Telegram Bot 推送', variable=tg_var,
                       font=('Microsoft YaHei', 10)).grid(
            row=r, column=0, columnspan=2, sticky='w', pady=4)
        r += 1
        tk.Label(f, text='TG Bot Token',
                 font=('Microsoft YaHei', 10), anchor='w').grid(
            row=r, column=0, sticky='w')
        tg_token_var = tk.StringVar(value=cfg.get('tgBotToken', ''))
        tk.Entry(f, textvariable=tg_token_var, width=40,
                 font=('Microsoft YaHei', 10), show='*').grid(row=r, column=1, padx=8)
        r += 1
        tg_hint = tk.Label(f, text='V5: Token从config.json读取，不再硬编码',
                           font=('Microsoft YaHei', 9), fg='#8b949e')
        tg_hint.grid(row=r, column=1, sticky='w')
        r += 1
        ...
        def save():
            ...
            c['enableTgBot'] = tg_var.get()
            c['tgBotToken'] = tg_token_var.get().strip()
            save_config(c)
            self.config = c
    ...
    def run(self):
        """启动应用。"""
        self.logbox.insert('end', '复盘助手 V5.0 启动\n')
