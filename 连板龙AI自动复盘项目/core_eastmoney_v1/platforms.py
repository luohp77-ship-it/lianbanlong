...
class EastMoneyPlatform(PlatformBase):
    ...
    def detect_path(self):
        """自动检测东方财富安装路径"""
        candidates = [
            "E:/东方财富/dfcf", "D:/东方财富/dfcf", "C:/东方财富/dfcf",
            "E:/东方财富", "D:/东方财富", "C:/东方财富",
            "C:/Program Files/东方财富",
            "C:/Program Files (x86)/东方财富",
        ]
        # 找主程序（stockway.exe 或 eastmoney.exe）
        for p in candidates:
            for fname in ["stockway.exe", "mainfree.exe", "eastmoney.exe", "EMWeb.exe"]:
                exe = os.path.join(p, fname)
                ...
                try:
                    ...
                    for i in range(100):
                        try:
                            ...
                            sub = winreg.OpenKey(key, subkey_name)
                            name = winreg.QueryValueEx(sub, "DisplayName")[0]
                            if "东方财富" in name or "eastmoney" in name.lower():
                                path = winreg.QueryValueEx(sub, "InstallLocation")[0]
                                if path and os.path.exists(path):
                                    return path
                        except:
    ...
    def blocknew_dir(self):
        """
        东方财富的自选股数据目录
        实测: config/User/guest/self_stock.db (SQLite数据库)
        """
        return os.path.join(self.data_dir, 'config', 'User', 'guest')

    def get_gridtab_path(self):
        """东方财富无gridtab概念"""
        return os.path.join(self.data_dir, 'config', 'User', 'guest', 'self_stock.db')

    def get_tdx_exe_path(self):
        detected = self.detect_path()
        if detected:
            for fname in ["stockway.exe", "mainfree.exe"]:
                exe = os.path.join(detected, fname)
                if os.path.exists(exe):
                    return exe
        cfg = self.load_config()
        base = cfg.get('eastmoneyDir', 'E:/东方财富')
        for fname in ["stockway.exe", "mainfree.exe"]:
            exe = os.path.join(base, 'dfcf', fname)
            if os.path.exists(exe):
                return exe
        return os.path.join(base, 'dfcf', "stockway.exe")
