#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""复盘助手 V3.1.0 - 通达信自动复盘桌面应用

V3.1.0变更（安全加固+QA修复）：
- SSL验证恢复：HTTPS默认验证证书，HTTP自动跳过
- blocknewDir 自动从 tdxDir 推导（防路径不一致）
- 板块配置文件原子写入（.tmp + os.replace）
- Word报告状态在总结行显示
- 新增 Cython + PyArmor 三层加密打包脚本
...
class FupanApp:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title('复盘助手 V3.1 - 通达信自动复盘')
    ...
    def run(self):
        """启动应用。"""
        self.logbox.insert('end', '复盘助手 V3.1.0 启动\n')
