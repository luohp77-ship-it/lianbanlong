#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""复盘引擎 V2.2.0：数据获取 → 连板计算(TDX本地) → 概念分析(reason_type) → 板块写入 → Word报告
...
def run_engine(config=None, callback=None):
    ...
    # ── 5. 概念分析（reason_type分组 + 综合评分重排）──
    log('  概念分析(reason_type分组+综合评分)...')
    ...
    n_extern = write_extern_user(tdx_dir, extern_stocks, code_concepts,
                                 lhb_formatted if lhb_formatted else None)
    if n_extern:
