...
def _add_styled_table(doc, headers, rows, col_widths=None, header_color='1A1A2E'):
    """创建统一风格的表格"""
...
def generate_word_report(
    ...
    # 晋级率
    yesterday_boards_count = 0  # 需要外部传入
    promo_rate = 0
    if lb_codes:
        # 晋级率 = 今日连板数/昨日涨停数（用board_days推断）
        # 简化：晋级涨停股中board>1的数量占总涨停比例
    ...
    disclaimer = (
        '本报告由复盘助手 V5 自动生成，数据来源于同花顺、东方财富、ClawHub 等公开数据平台。\n\n'
